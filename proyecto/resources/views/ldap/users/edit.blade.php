...
@section('title', 'Editar Usuario LDAP')

@section('content')
<div class="container mx-auto px-4 py-8 animated-fade-in">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-blue-900">Editar Usuario LDAP</h1>
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="p-6">
            <form action="{{ route('ldap.users.update', ['dn' => urlencode($user->getDn())]) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label for="uid" class="block text-sm font-medium text-gray-700 mb-1">Usuario (UID)</label>
                        <input type="text" name="uid" id="uid" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed" 
                            value="{{ $user->getAttribute('uid')[0] ?? '' }}" readonly disabled>
                        <p class="text-sm text-gray-500 mt-1">El UID no puede modificarse</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="cn" id="cn" required 
                            ...
                            value="{{ old('cn', $user->getAttribute('cn')[0] ?? '') }}" placeholder="Nombre completo">
                        @error('cn')
                    ...
                    <div>
                        ...
                        <input type="text" name="sn" id="sn" required 
                            ...
                            value="{{ old('sn', $user->getAttribute('sn')[0] ?? '') }}" placeholder="Apellido">
                        @error('sn')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="mail" id="mail" required 
                            ...
                            value="{{ old('mail', $user->getAttribute('mail')[0] ?? '') }}" placeholder="correo@ejemplo.com">
                        @error('mail')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('password')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <p class="text-sm text-gray-500 mt-1">Mínimo 8 caracteres. Dejar en blanco para mantener la contraseña actual.</p>
                    </div>
                        ...
                        @foreach($groups as $group)
                            <div class="flex items-center">
                                <input type="checkbox" name="groups[]" id="group_{{ $loop->index }}" 
                                    ...
                                    {{ in_array($group->getDn(), $userGroups) ? 'checked' : '' }}>
                ...
                <div class="border-t border-gray-200 pt-4 flex justify-between">
                    <a href="{{ route('ldap.users.show', ['dn' => urlencode($user->getDn())]) }}" class="bg-gray-200 text-gray-700 py-2 px-6 rounded-lg hover:bg-gray-300 transition duration-300">
